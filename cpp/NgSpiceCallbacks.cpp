...
int ng_sync(double actual_time, double *delta_time, double old_delta_time, int redostep, int identification_number, int location, void *user_data) {
    ...
    DBG("actual_time=%g time_spice=%lld delta_time=%g delta_time_spice=%lld old_delta_time=%g redostep=%d identification_number=%d location=%d next_spice_step=%lld get_time(1)=%lld", actual_time,
        time_spice, *delta_time, delta_time_spice, old_delta_time, redostep, identification_number, location, g_time_barrier.get_next_spice_step_time(), g_time_barrier.get_time(spice_vpi::TimeBarrier<unsigned long long>::SPICE_ENGINE_ID));

    // DBG("time_spice=%f time_spice=%f", actual_time*g_config.time_precision, *delta_time * g_config.time_precision);

    if (location == 1 and g_time_barrier.needs_redo()) {

        // Skip redo since the actual step is before next time step? -> CHECK
        if (time_spice < g_time_barrier.get_time(spice_vpi::TimeBarrier<unsigned long long>::SPICE_ENGINE_ID)) {
            DBG("cancel redo");
            return 0;
        }

        unsigned long long old_delta_time_spice = std::llround(old_delta_time * g_config.time_precision);
        unsigned long long new_delta_time_spice = old_delta_time_spice - (time_spice - g_time_barrier.get_time(spice_vpi::TimeBarrier<unsigned long long>::SPICE_ENGINE_ID));

        double redo_time_db = ((double)g_time_barrier.get_time(spice_vpi::TimeBarrier<unsigned long long>::SPICE_ENGINE_ID)) * 1.0/g_config.time_precision;
...
int ng_srcdata(double *vp, double time, char *source, int id, void *udp) {
    ...
    // unsigned long long time_spice_to_vpi = (unsigned long long)(time * 1000000000000ULL);
    unsigned long long time_spice_to_vpi = std::llround(time * g_config.time_precision);

    DBG("source=%s vp=%g get_time(1)=%lld time_spice=%lld redo_step=%d  time_spice_to_vpi=%lld", source, *vp, g_time_barrier.get_time(spice_vpi::TimeBarrier<unsigned long long>::SPICE_ENGINE_ID), time_spice_to_vpi, g_time_barrier.needs_redo(), time_spice_to_vpi);
    ...
    DBG("source=%s time=%g vp=%g time_spice_to_vpi=%lld time_ns=%g", source, time, *vp, time_spice_to_vpi, time * 1e9);
